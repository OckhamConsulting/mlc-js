...
describe("vote.js", function () {
  ...
  describe("#createVote", function () {
    var createVote = vote.createVote,
      vt = null,
      publicKey = mlc.crypto.getKeys("secret").publicKey,
    ...
    it("should create vote from ecpair", function () {
      ...
      var secondSecretKey = mlc.ECPair.fromSeed("second secret");
    ...
    it("should create vote from wif", function () {
      var secretKey = mlc.ECPair.fromWIF("SB3iDxYmKgjkhfDZSKgLaBrp3Ynzd3yd3ZZF2ujVBK7vLpv6hWKK", NETWORKS.mlc);
      secretKey.publicKey = secretKey.getPublicKeyBuffer().toString("hex");
    ...
    it("should be deserialised correctly", function () {
      var deserialisedTx = mlc.crypto.fromBytes(mlc.crypto.getBytes(vt).toString("hex"));
    ...
    describe("returned vote", function () {
      ...
      it("should have recipientId string equal to sender", function () {
        (vt).should.have.property("recipientId").and.be.type("string").and.equal(mlc.crypto.getAddress(publicKey))
      });
      ...
      it("should be signed correctly", function () {
        var result = mlc.crypto.verify(vt);
        (result).should.be.ok;
      });

      it("should be second signed correctly", function () {
        var result = mlc.crypto.verifySecondSignature(vt, mlc.crypto.getKeys("second secret").publicKey);
        (result).should.be.ok;
      });
