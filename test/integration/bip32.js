...
describe('mlc-js (BIP32)', function () {
  it('can create a BIP32 wallet external address', function () {
    var path = "m/0'/0/0"
    var root = mlc.HDNode.fromSeedHex('dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd')
  ...
  it('can create a BIP44, mlc, account 0, external address', function () {
  ...
  it('can recover a BIP32 parent private key from the parent public key, and a derived, non-hardened child private key', function () {
    function recoverParent (master, child) {
      ...
      for (var i = 0; i < mlc.HDNode.HIGHEST_BIT; ++i) {
        ...
        var Qp = new mlc.ECPair(d2).Q
        if (Qp.equals(QP)) break
      }

      var node = new mlc.HDNode(new mlc.ECPair(d2), master.chainCode, master.network)
    ...
    var master = mlc.HDNode.fromSeedBuffer(seed)
