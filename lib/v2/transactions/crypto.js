...
function getBytes(transaction) {
	...
	bb.writeByte(transaction.network); // ark = 0x17, devnet = 0x30, mlc = ?
...
function getKeys(secret, network) {
	var ecpair = ECPair.fromSeed(secret, network || networks.mlc);
